...
export default function Dashboard({ session }) {
    ...
    // Show centered toast for 3s when a new notification arrives
    useEffect(() => {
        ...
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row font-sans text-slate-800">
            ...
            {toast && (
                <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
                    <div className="pointer-events-auto bg-white rounded-2xl shadow-2xl border border-slate-100 px-8 py-6 flex flex-col items-center gap-3 max-w-sm w-full mx-4"
                        ...
                        <p className="text-[17px] font-bold text-slate-900 text-center">{toast.title}</p>
                        <p className="text-sm text-slate-500 text-center leading-relaxed">{toast.message}</p>
                        {/* 3s progress bar */}
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                <header className="h-20 bg-white border-b border-slate-100 hidden md:flex items-center justify-between px-8 shrink-0">
                    <div className="flex flex-col">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Intelligence Center</p>
                    </div>
                    <div className="flex items-center gap-4 ml-auto mt-4 md:mt-0">
                        {/* Credit pill */}
                        <button
                            onClick={() => navigate('/buy-credits')}
                            className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 rounded-full transition-colors"
                        >
                            <Coins size={14} className="text-indigo-500" />
                            <span className="text-xs font-bold text-indigo-700">{session?.user?.user_metadata?.credits ?? 1000} credits</span>
                        </button>
                        ...
                        <div className="relative">
                            {(() => {
                                const typeStyles = {
                                    credit: { bg: 'bg-emerald-100', emoji: '🪙' },
                                    debit: { bg: 'bg-rose-100', emoji: '💸' },
                                    info: { bg: 'bg-blue-100', emoji: '💡' },
                                    success: { bg: 'bg-indigo-100', emoji: '✅' },
                                };
                                ...
                                return (
                                    <div ref={bellRef}>
                                        <button
                                            ...
                                            className="relative w-9 h-9 flex items-center justify-center rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors"
                                        >
                                            <Bell size={20} />
                                            {unreadCount > 0 && (
                                        ...
                                        {bellOpen && (
                                            <div className="absolute right-0 top-11 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-in slide-in-from-top-2 duration-200">
                                                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                                                    <span className="text-sm font-bold text-slate-800">Notifications</span>
                                                    {notifications.length > 0 && (
                                                        <button onClick={markAllRead} className="text-[11px] font-semibold text-indigo-500 hover:text-indigo-700">Mark all read</button>
                                                    )}
                                                </div>
                                                    ...
                                                    {notifications.length === 0 ? (
                                                        ...
                                                        return (
                                                            <div key={n.id} className={`flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors ${!n.read ? 'bg-indigo-50/40' : ''}`}>
                                                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-base ${style.bg}`}>{style.emoji}</div>
                        ...
                        <div className="relative pl-4 border-l border-slate-200">
                            <button
                                ...
                                className="flex items-center gap-3 hover:bg-slate-50 p-1.5 rounded-xl transition-all duration-200"
                            >
                            ...
                            {profileOpen && (
                                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 z-50 animate-in slide-in-from-top-2 duration-200">
                                    <button
                                        onClick={() => navigate('/profile')}
                                        className="w-full flex items-center gap-2 px-3 py-2 text-slate-600 hover:bg-slate-50 rounded-xl transition-colors font-semibold text-xs text-left"
                                    >
                                        <User size={14} /> My Profile
                                    </button>
                                    <button
                                        onClick={handleLogout}
                                        className="w-full flex items-center gap-2 px-3 py-2 text-red-500 hover:bg-red-50 rounded-xl transition-colors font-bold text-xs text-left"
                                    >
                                                    ...
                                                    {gaps.sort((a, b) => b.gap_score - a.gap_score).map((gap, i) => (
                                                        <tr key={gap.id}>
                                                            ...
                                                            <td className={`py-4 text-right ${i !== 0 ? 'border-t border-slate-50' : ''}`}>
                                                                <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${gap.gap_score > 20 ? 'bg-rose-50 text-rose-500' :
